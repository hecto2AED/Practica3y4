...
pr_tidy <- pew_religion %>%
  gather(key = "income", value = "value", -religion) %>%
...
raw_tidy <- raw %>% 
  pivot_longer(
    cols = starts_with("wk"), # indica las columnas con las que queremos apilar
    names_to = "week", #--> aquí marcaremos el nombre de la nueva columna que agrupa todas las semanas (que son los nombre de las columnas que estamos apilando)
    values_to = "rank", #--> aquí decimos el nombre de la columna que almacena el valor para cada semana, en este caso el rango o rank
    names_prefix = "wk" # aquí decimos que queremos quitar wk y quedarnos con el número de la semana (ya que no hace falta especificar que son semanas pues ya estan en la columna semanas)
  ) %>%
  mutate(
...
weather_tidy <- raw %>%
  pivot_longer(
    cols = starts_with("d"), #elegimos las columnas que queremos apilar en este caso los días
    names_to = "día", #nombre de la nueva columna que contiene la variable día
    values_to = "value",
    values_drop_na = TRUE #quitamos los NA que hay muchos que no valen para nada
  ) %>%
  pivot_wider( #wider vale para separar los valores de la columna element en columnas distintas, es decir que pasen de filas a columnas
    names_from = element, # elegimos los names from columna elemento
    values_from = value # les asignamos sus valores de la columna value
  ) %>%
  mutate(
    día = as.integer(str_remove(día, "d")),
    fecha = as.Date(paste(year, month, día, sep = "/"))
  )
...
#Uno contiene información sobre los vuelos y el otro sobre los propios aviones de dichos vuelos, se pueden relacionar con la columna tailum.
...
planes.f <- planes %>%
  filter(!str_detect(toupper(manufacturer), "MCDONELL DOUGLAS")) %>% # aquí lo que hacemos es pillar todos los macdonel y descartarlos, coger todo menos eso
  filter(seats >= 90, seats <= 210) %>%
  mutate(manufacturer = ifelse(str_detect(toupper(manufacturer), "AIRBUS"), "AIRBUS COMP", manufacturer))
...
flights <- flights %>%
  mutate(
    Full_Date = paste(day, month, year.x, sep = "-")
  ) %>%
  select(
    -day, -month, -year.x
  )
